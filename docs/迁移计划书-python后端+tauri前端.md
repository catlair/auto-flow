...
- 全量：Python **75 passed**、前端 20 passed、`vue-tsc` 通过。

**遗留**：Quartz 兜底路径（mss 枚举不到显示器时）拿到的是所有屏的并集位图，
只能有一个全局 scale，混合 Retina 下该路径存在已知近似；正常路径不受影响。

**顺带修掉的一个静默点错**（做多屏时在真机上撞到的）：纯色图会让
`TM_CCOEFF_NORMED` 变成 0/0 的退化输入，而 OpenCV 的结果**不确定**——实测同一个
「纯色屏 + 纯色 40x60 模板」，有的进程跑出 `max=1.0 @ (0,0)`（满分假命中），
有的跑出 `0.0`。真机上复现过一次：没给「屏幕录制」权限时 mss 返回全均匀图
（`std=0.000`，此时 scale 往往还是 2.0），`find_template` 于是报
`found=True score=1.0000 落在 (0,0)` —— 工作流会点向屏幕左上角且不报错。

现在 `_is_flat()`（`std < 1.0`）拦两处：模板纯色直接报「找不到」，
屏幕纯色则跳过该屏并**记一次 WARNING**（点明最常见成因是缺屏幕录制权限）——
此前权限缺失只表现为「节点找不到目标」，用户无从判断是权限问题。
...
**但中文往返仍然成立（同次实测，2026-09-12）**：把录下的那 12 条拼音按键原样回放，
...
准确说法是"**录得到拼音、放得出中文，但不确定**"——依赖回放瞬间输入法是否开启、
候选顺序是否一致；输入法没开就退化成字面量 `nihao`。所以：

- **确定性回放**：用 `record.keysToText` 把那段按键替换成一条 `text` 事件（回放走
  `type_text` 的 Unicode 通道，不经过输入法）。文字由用户填——IME 提交了什么，
  事件层无从得知，不能猜。面板上对应"这段按键 / 替换为文本"输入块。
- **简单直接**：中文用「**键盘输入**」节点（`mode="text"`）填写，不依赖录制。
- **录制侧直接拿到汉字**的唯一可行方向是 AX 轮询 `kAXValueAttribute` 差分；但它对
  终端/画布类应用不适用、也无法知道插入位置，需单独评估后再决定（当前未做）。
