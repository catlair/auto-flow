...
| **流程图模型** | [flow.md](modules/flow.md) | ✅ | 176 测试 + 90 前端用例 | **v4：节点 + 有向边**；出口名分派、汇合、循环上限、旧文件迁移；节点改动按 uid 寻址 |
...
| 前端界面 | [frontend.md](modules/frontend.md) | ✅ | 真机目视 + 90 前端用例 | v4 画布（@vue-flow/core）；响应驱动刷新；诊断面板显示后端告警；`fitOnce` 管视野适应时机；连线落点左/上/下三边 |
