...
| 模块 | 文档 | 状态 | 验收 | 备注 |
| --- | --- | --- | --- | --- |
| 键鼠录制 | [recorder.md](modules/recorder.md) | ✅ | 153 测试 + 真机 | v3 重写：保轨采样 + 拖拽/双击语义 + 事件编辑 + 文本聚合 |
| 回放引擎 | [player.md](modules/player.md) | ✅ | 153 测试 + 真机 | v3 重写：虚拟时钟 + 三档追赶 + 文本投递 |
...
| RPC 协议 | [rpc-protocol.md](modules/rpc-protocol.md) | ✅ | 153 测试 | 写锁防交错；广播 events 摘要化；后端 WARNING 转发为 `log.warning` |
| 前端界面 | [frontend.md](modules/frontend.md) | ✅ | 真机 + 49 前端用例 | 响应驱动刷新；通知仅广播冗余；诊断面板显示后端告警 |
| 打包/签名/部署 | [packaging.md](modules/packaging.md) | ✅ | 16 项产物/RPC 断言 | ⚠️ 修复后必须重打包部署；第 5 步 `check_installed.py` 收口 |
