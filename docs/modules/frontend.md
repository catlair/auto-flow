...
- **F-UI-02 出口标签不被手柄遮挡**（2026-09-15）：3 例源文件结构断言
  （`.af-exit` 不得为定位元素且须有 `padding-right`、`.af-gnode` 须为定位元素、
  模板须把 `nodes-initialized` 接到 `tryFit`）。反向验证：把
  `FlowCanvas.vue` 还原到修复前，恰好这 3 例失败、其余全绿。
...
6. **画布是「投影」，不是第二份真源**（F-UI-02）。`vNodes` / `vEdges` 两个 computed
   ...
   **不做乐观修改**——乐观改边之后如果后端拒绝（自环、节点已删），画布会停在和
   ...
   - **手柄位置靠 `position: relative` 的节点卡片**。库自带的
     `.vue-flow__handle-right` 是 `top:50%; right:0; translate(50%,-50%)`，锚在
     **最近的定位祖先**的右边缘。所以 `.af-gnode`（卡片本体）必须是定位元素，
     手柄才落在卡片右边缘上；不写的话它会一路锚到 `.vue-flow__node` 上，
     单出口与多出口的竖线对不齐。
     ⚠️ **出口行（`.af-exit`）绝对不能写 `position: relative`**——那会把锚点从
     卡片挪到这一行上，手柄内缩约 10px 并**正好压住标签最后一个字**：
     「情形 1 / 情形 2」的数字被圆点盖掉，用户分不清哪条边是情形几。
     给手柄让位改用 `padding-right: 8px`（手柄半径 + 余量），**不是**定位。
     不用覆盖库的样式，也就不会随库升级失效。
