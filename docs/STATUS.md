...
| 模块 | 文档 | 状态 | 验收 | 备注 |
| --- | --- | --- | --- | --- |
| 键鼠录制 | [recorder.md](modules/recorder.md) | ✅ | 174 测试 + 真机 | v3 重写：保轨采样 + 拖拽/双击语义 + 事件编辑 + 文本聚合 |
| 回放引擎 | [player.md](modules/player.md) | ✅ | 174 测试 + 真机 | v3 重写：虚拟时钟 + 三档追赶 + 文本投递 |
| **流程图模型** | [flow.md](modules/flow.md) | ✅ | 174 测试 + 83 前端用例 | **v4：节点 + 有向边**；出口名分派、汇合、循环上限、旧文件迁移；节点改动按 uid 寻址 |
...
| RPC 协议 | [rpc-protocol.md](modules/rpc-protocol.md) | ✅ | 174 测试 | 写锁防交错；广播 events 摘要化；后端 WARNING 转发为 `log.warning`；节点改动按 uid 寻址 |
| 前端界面 | [frontend.md](modules/frontend.md) | ✅ | 真机目视 + 83 前端用例 | v4 画布（@vue-flow/core）；响应驱动刷新；诊断面板显示后端告警；`fitOnce` 管视野适应时机 |
| 打包/签名/部署 | [packaging.md](modules/packaging.md) | ✅ | 38 项产物/RPC 断言 | ⚠️ 修复后必须重打包部署；第 5 步 `check_installed.py` 收口 |
