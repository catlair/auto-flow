...
- `tauri/src/flow/cycle.ts` — 环检测纯函数（`pathTo` 可达路径 / `cycleEdges` 环上的边），
  无 Vue/DOM 依赖
...
| F-FLW-16 | 连线成环时提示（**不拦**），并给出环上的边 | ✅ | 连上那一刻判「`dst` 能否走回 `src`」，是则提示「出现了环，环上的连线已标红，确认是想要的循环而不是连错」；回边合法，**不阻止落库** |
...
- **F-FLW-16 目视**（2026-09-15）：改的是提示与边的呈现，按项目规则走目视验收——
  ...
  灰态对照 0）、点空白后红像素归零。细节见 `docs/modules/frontend.md`。
- **整体**：`pytest tests/ -q` → **176 passed**；`npm --prefix tauri test` → **113 passed**
...
- **成环提示只覆盖「手动连线」这一条路径**（2026-09-15 起，F-FLW-16）。
  **打开文件时不检查图里已有的环**——这是**刻意的**（理由见设计要点 18：
  ...
  所以剩下的盲区只有「手工编辑过的 json / 从别处拿到的文件」。
  另外**静态判定「这个环是死循环」是不可判定的**（分支没被选中的 `case` 上的环
  永远不会走到），所以提示只能停在「有环，确认一下」。
- **环上的边只在「刚连上」之后标红，下一次交互就清掉**。所以它帮不了
  「打开一份别人的文件、想看看环在哪」这种场景——那时候画布上不会有任何标记。
  要覆盖它得引入「常驻标出所有环」的开关（默认关），现在没做。
