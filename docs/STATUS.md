...
| 模块 | 文档 | 状态 | 验收 | 备注 |
| --- | --- | --- | --- | --- |
| 键鼠录制 | [recorder.md](modules/recorder.md) | ✅ | 191 测试 + 真机 | v3 重写：保轨采样 + 拖拽/双击语义 + 事件编辑 + 文本聚合 |
| 回放引擎 | [player.md](modules/player.md) | ✅ | 191 测试 + 真机 | v3 重写：虚拟时钟 + 三档追赶 + 文本投递 |
| **流程图模型** | [flow.md](modules/flow.md) | ✅ | 191 测试 + 139 前端用例 | **v4：节点 + 有向边**；出口名分派、汇合、循环上限、旧文件迁移；节点改动按 uid 寻址；连线成环当场提示并把环标出来（不拦） |
| 工作流执行器 | [executor.md](modules/executor.md) | ✅ | 测试 | v4 图遍历（出口分派 + 步数上限）+ 异常兜底 + 热停 |
| 节点体系 | [nodes.md](modules/nodes.md) | ✅ | 测试 + 真机 | 13 种内置节点（含 start / condition_group / branch / end），自描述参数 + show_if；`delay` 按 `mode` 互斥显示两种等待方式 |
...
| RPC 协议 | [rpc-protocol.md](modules/rpc-protocol.md) | ✅ | 191 测试 | 写锁防交错；广播 events 摘要化；后端 WARNING 转发为 `log.warning`；节点改动按 uid 寻址 |
| 前端界面 | [frontend.md](modules/frontend.md) | ✅ | 真机目视 + 139 前端用例 | 画布主导 + 标签侧栏（参数/运行/录制，宽度可拖、可收起、状态持久化）；v4 画布（@vue-flow/core）；响应驱动刷新；诊断面板显示后端告警；`fitOnce` 管视野适应时机；连线落点左/上/下三边；连线成环提示 + 标红；「标出环」常驻开关（默认关） |
...
- 📋 条件分支扩展：比较类条件（变量 / 上一节点结果）而非仅「存在性检测」。
  「与 / 或 组合多个检测」已由 `condition_group` 覆盖，剩下的是**比较**这一类
